module ClosureTree
  module ActsAsTree
    def acts_as_tree(options = {})
      ...
      class_inheritable_accessor :closure_tree_options
      ...
      class_inheritable_accessor :hierarchy_class
      ...
      named_scope :leaves, :conditions => (" #{quoted_table_name}.#{primary_key} IN
        (SELECT ancestor_id
         FROM #{quoted_hierarchy_table_name}
         GROUP BY 1
         HAVING MAX(generations) = 0)")
  ...
  module Model
    ...
    module InstanceMethods
      ...
      def leaves
        ...
        #        SQL
        #        )
        self.class.leaves.find(:all, :conditions => 
        "#{quoted_table_name}.#{self.class.primary_key} IN (
            SELECT descendant_id
            FROM #{quoted_hierarchy_table_name}
            WHERE ancestor_id = #{id})"
      ...
      def self_and_siblings
        ...
        self.class.find_all_by_parent(parent)
      end
      ...
      def find_or_create_by_path(path, attributes = {})
        ...
        path.each do |name|
          ...
          child = node.children.find(:first, :conditions => attrs)
          unless child
